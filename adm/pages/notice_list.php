...
<div class="con_wrap">
				...
				<li class="search_input">
					<div>
						<select name="f_kind" id="" class="">
							...
							<option value="n_title" <?if($f_kind=='n_title')echo 'selected'?>>제목</option>
							<option value="n_content" <?if($f_kind=='n_content')echo 'selected'?>>내용</option>
						</select>
					</div>
					<div>
	...
	$whereStr = " WHERE n_state in (0, 1, 2) ";
	...
	if($f_start!=''){
		...
		$whereStr .= " AND n_reg_date >= '".date('Y-m-d', $start_date)." 00:00:00' "; 
	}
	...
	if($f_end!=''){
		...
		$whereStr .= " AND n_reg_date < '".date('Y-m-d', $end_date)." 00:00:00' "; 
	}
	...
	<div class="table_wrap mt40">
		...
		<table>
			...
			?>
				...
				<tr>
					...
					<td class="tal" ><a href="./?abkind=<?=$abkind?>&amkind=edit&idx=<?=$row["idx"];?>&f_start=<?=$f_start;?>&f_end=<?=$f_end;?>&f_kind=<?=$f_kind;?>&f_txt=<?=$f_txt;?>&page=<?=$page;?>"><?=$row["n_title"];?></a></td>
					<td>
						<?
						switch ($row["n_state"]) {
							case '2':
					...
					<td><?=substr($row["n_reg_date"], 0, 16);?></td>
					<td><?=substr($row["n_edit_date"], 0, 16);?></td>
				</tr>
		...
		<div class="foot_btn">
			<a href="./?abkind=notice&amkind=reg" class="b_add b_blue">등록</a>
		</div>
